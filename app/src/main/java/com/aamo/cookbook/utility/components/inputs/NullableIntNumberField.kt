...
fun NullableIntNumberField(
  ...
  TextField(
    value = TextFieldValue(
      text = value?.toString() ?: String.EMPTY, selection = selection
    ),
    shape = shape,
    colors = colors,
    placeholder = placeholder,
    onValueChange = {
      // TODO: change to IntNumberFieldValue
      if (it.text.isEmpty() || (zeroEqualsNull && it.text == "0")) {
        onValueChange(null)
      }
      else {
        when {
          ...
          if (newValue != value) {
            ...
            selection =
              it.selection.letIf(restrictSelectionOnZero && newValue == 0) { TextRange(1) }
          }
        }
      }
    },
