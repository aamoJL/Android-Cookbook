...
import com.aamo.cookbook.utility.extensions.sharedViewModel
...
fun NavGraphBuilder.editRecipeGraph(
  ...
  navigation(
    ...
    composable(route = EditRecipeScreenPage.EditInfo.route, enterTransition = {
      if (this.initialState.destination.route != EditRecipeScreenPage.EditChapter.route) this.primaryEnterTransition()
      else secondaryEnterTransition()
    }, exitTransition = {
      if (this.targetState.destination.route == EditRecipeScreenPage.EditChapter.route) primaryExitTransition()
    ...
    composable(route = EditRecipeScreenPage.EditChapter.route, enterTransition = {
      if (this.initialState.destination.route == EditRecipeScreenPage.EditInfo.route) this.primaryEnterTransition()
      else secondaryEnterTransition()
    }, exitTransition = {
      if (this.targetState.destination.route == EditRecipeScreenPage.EditStep.route) primaryExitTransition()
      else this.secondaryExitTransition()
    }) {
      val editRecipeViewModel =
        it.sharedViewModel<EditRecipeViewModel>(navController = navController)

      EditRecipeChapterScreen(viewModel = editRecipeViewModel, onEditStep = { index ->
    ...
    composable(route = EditRecipeScreenPage.EditStep.route, enterTransition = {
      if (this.initialState.destination.route == EditRecipeScreenPage.EditChapter.route) this.primaryEnterTransition()
      else secondaryEnterTransition()
    }, exitTransition = {
      if (this.targetState.destination.route == EditRecipeScreenPage.EditIngredient.route) primaryExitTransition()
      else this.secondaryExitTransition()
    }) {
      val editRecipeViewModel =
        it.sharedViewModel<EditRecipeViewModel>(navController = navController)

      EditRecipeChapterStepScreen(viewModel = editRecipeViewModel, onEditIngredient = { index ->
    ...
    composable(
      ...
      EditRecipeChapterStepIngredientScreen(viewModel = editRecipeViewModel, onSubmitChanges = {
