...
class OnChange {
  ...
  fun onChange() {
    var called = 0
    val state =
      RecipeFormViewModel.FormIngredientState(id = UUID.randomUUID(), onChange = { called++ })

    state.name.update("Name").also { Assert.assertEquals(1, called) }
    state.amount.update(3.3).also { Assert.assertEquals(2, called) }
    state.unit.update("Unit").also { Assert.assertEquals(3, called) }
  }
}