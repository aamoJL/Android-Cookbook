...
class RecipeSearchViewModelTest {
  ...
  fun verifyInitValidRecipes() {
    ...
    val actual = viewModel.validRecipes.value.map { it.recipe }
    assertEquals(expected, actual)
  }

  @Test
  fun setSearchWord() {
    val expected = viewModel.validRecipes.value.first().recipe.name

    viewModel.setSearchWord(expected)
    assertEquals(1, viewModel.validRecipes.value.size)

    val actual = viewModel.validRecipes.value.first().recipe.name
    assertEquals(expected, actual)
  }
}