...
package com.aamo.cookbook.ui_tests.ui.components.inputs.number_field.float_number_field.float_field_validator
...
class ValidateText {
  ...
  fun `validate valid text`() {
    ...
    inputOutputs.forEach { (input, outputs) ->
      ...
      FloatFieldValidator.onValid(text = input) { v, t -> value = v; text = t }
  ...
  fun `validate invalid text`() {
    ...
    inputs.forEach { input ->
      FloatFieldValidator.onValid(text = input) { _, _ -> fail() }
    }
  }
}