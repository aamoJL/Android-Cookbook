...
class StepTimerField : PageTest() {
  ...
  @Test
  fun `timer field toggle`() = runTest {
    rule.onNodeWithText(getString(R.string.label_step_timer).asOptionalLabel()).isNotDisplayed()
    rule.onNodeWithContentDescription(getString(R.string.cd_toggle_timer_field)).performClick()
    rule.onNodeWithText(getString(R.string.label_step_timer).asOptionalLabel()).isDisplayed()
  }
}