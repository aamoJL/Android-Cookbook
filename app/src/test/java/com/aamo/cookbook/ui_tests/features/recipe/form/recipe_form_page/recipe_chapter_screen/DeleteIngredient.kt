...
class DeleteIngredient : PageTest() {
  @Before
  fun setup() = runTest {
    toRecipeFormPage()
    waitForLoading()
  ...
  fun `delete chapter`() = runTest {
    rule.onNodeWithText(getString(R.string.title_ingredients)).isDisplayed()
    rule.onNodeWithContentDescription(getString(R.string.cd_delete_ingredient)).performClick()
    rule.onNodeWithText(getString(R.string.title_ingredients)).assertDoesNotExist()
  }
}